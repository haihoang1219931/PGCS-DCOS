...
void fhog(const cv::Mat &image, vector<cv::Mat> &fhogs, int binSize, int orientations)
{
    assert(image.type() == CV_32F || image.type() == CV_32FC3);
    ...
    for (size_t i = 0; i < nChannls; i++) {
        cv::Mat tmp(cv::Size(wb, hb), CV_32FC1);
...
void fhog(const cv::Mat &image, cv::Mat &fhogs, int binSize, int orientations)
{
    assert(image.type() == CV_32F || image.type() == CV_32FC3);
    float *M = new float[image.rows * image.cols];
    float *O = new float[image.rows * image.cols];
    ...
    gradientMagnitude(image, M, O);
    fhog(M, O, H, image.rows, image.cols, binSize, orientations, -1, 0.2f);
    fhogs = cv::Mat::zeros(cv::Size(wb, hb), CV_32FC(nChannls));
...
{
    assert(image.type() == CV_32F || image.type() == CV_32FC3);
    M = cv::Mat::zeros(image.size(), CV_32FC1);
    O = cv::Mat::zeros(image.size(), CV_32FC1);
    float *tI = new float[image.rows * image.cols * image.channels()];
