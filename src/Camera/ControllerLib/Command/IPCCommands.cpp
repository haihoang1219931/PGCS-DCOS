...
void IPCCommands::changeSensorID(QString sensorID)
{
    ...
    m_buffer->add(packetEncoded);
    m_buffer->send();
    m_gimbalModel->gimbal()->m_sensorMode = sensorID.toStdString();
}
void IPCCommands::changeLockMode(QString lockMode, QString geoLocation)
{
    ...
    m_buffer->add(packetEncoded);
    m_buffer->send();
    m_gimbalModel->gimbal()->m_lockMode = tmpLockMode.toStdString();
}
void IPCCommands::takeSnapshot(QString mode, int frameID)
{
    ...
    m_buffer->add(packetEncoded);
    m_buffer->send();
    m_gimbalModel->gimbal()->m_snapShotMode = mode.toStdString();
}
void IPCCommands::changeSensorColor(QString mode)
{
    ...
    m_buffer->add(packetEncoded);
    m_buffer->send();
    m_gimbalModel->gimbal()->m_sensorColorMode = mode.toStdString();
}
...
void IPCCommands::changeGimbalMode(QString mode)
{
    ...
    m_buffer->add(packetEncoded);
    m_buffer->send();
    m_gimbalModel->gimbal()->m_gimbalMode = mode.toStdString();
}
                                ...
                                double height, double objW, double objH)
{
    ...
    packet.key = (key_type)EyePhoenixProtocol::ScreenPress;
    packet.data = data.toByte();
    vector<byte> packetEncoded = packet.encode();
    m_buffer->add(packetEncoded);
    m_buffer->send();
...
void IPCCommands::enableImageStab(QString mode, double cropRatio)
{
    ...
    m_buffer->add(packetEncoded);
    m_buffer->send();
    m_gimbalModel->gimbal()->m_videoStabMode = _mode;
}
...
void IPCCommands::treronChangeSensor(QString sensorID){
    if(sensorID == "EO"){
        m_gimbalModel->m_gimbal->m_sensorID = 0;
    }else{
        m_gimbalModel->m_gimbal->m_sensorID = 1;
    }
}
void IPCCommands::treronZoomIn(){
    unsigned char dataBytes[] = {0x06,0x81,0x01,0x04,0x07,0x27,0xFF,0x00,0x00,0x00};
    vector<unsigned char> data(dataBytes, dataBytes + 10);
    m_tcpSensor->sendData(data);
}

void IPCCommands::treronZoomOut(){
    unsigned char dataBytes[] = {0x06,0x81,0x01,0x04,0x07,0x37,0xFF,0x00,0x00,0x00};
    vector<unsigned char> data(dataBytes, dataBytes + 10);
    m_tcpSensor->sendData(data);
}

void IPCCommands::treronZoomStop(){
    unsigned char dataBytes[] = {0x06,0x81,0x01,0x04,0x07,0x00,0xFF,0x00,0x00,0x00};
    vector<unsigned char> data(dataBytes, dataBytes + 10);
    m_tcpSensor->sendData(data);
}

void IPCCommands::treronGetZoomPos(){
    unsigned char dataBytes[] = {0x05,0x81,0x09,0x04,0x47,0xFF,0x00,0x00,0x00,0x00};
    vector<unsigned char> data(dataBytes, dataBytes + 10);
    m_tcpSensor->sendData(data);
}

void IPCCommands::gimbalControl(int _rollSpeed, int _panSpeed, int _tiltSpeed){
    s16_Gimbal_Control(m_tcpGimbal, _rollSpeed, _tiltSpeed, _panSpeed);
}
