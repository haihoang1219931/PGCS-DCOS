...
namespace math
{
	...
	public:
		...
		//EyePhoenix: Add to byte
                ...
                inline std::vector<Eye::byte> toByte();
                inline Matrix_<T>* parse(V_INT_16 _row, V_INT_16 _col, std::vector<Eye::byte> _data, index_type _index=0);
                inline Matrix_<T>* parse(V_INT_16 _row, V_INT_16 _col, Eye::byte* _data, index_type _index = 0);
...
//		return Matrix_<T>(cols, rows, _data);
	...
	template<typename T>
        inline std::vector<Eye::byte> Matrix_<T>::toByte()
	{
                std::vector<Eye::byte> _result(0);
                std::vector<Eye::byte> b_vector;
		T value;
	...
	template<typename T>
        inline Matrix_<T>* Matrix_<T>::parse(V_INT_16 _row, V_INT_16 _col, std::vector<Eye::byte> _data, index_type _index)
	{
		Matrix_<T> tmp(_row, _col);
		this->swap(tmp);
                Eye::byte* data = _data.data();
		T * ptr = &this->_Mimpl[0];
		index_type index = 0;
	...
	template<typename T>
        inline Matrix_<T>* Matrix_<T>::parse(V_INT_16 _row, V_INT_16 _col, Eye::byte* _data, index_type _index)
	{
		Matrix_<T> tmp(_row, _col);
		this->swap(tmp);
		T * ptr = &this->_Mimpl[0];
