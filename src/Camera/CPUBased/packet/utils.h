...
namespace Utils
{
	...
	{
		T value;
                Eye::byte data[sizeof(T)];
	};

	template<typename T>
        static std::vector<Eye::byte> toByte(T input, Endianness endianess = Endianness::Little)
	{
                std::vector<Eye::byte> result;
		b_T<T> tmp;
		tmp.value = input;
	...
	template<typename T>
        static T toValue(Eye::byte *bytes, int startIndex, Endianness endianess = Endianness::Little)
	{
		b_T<T> tmp;
		//printf("Parsing byte *...\r\n");
		if (endianess == Endianness::Big){
	...
	template<typename T>
        static T toValue(std::vector<Eye::byte> bytes, int startIndex, Endianness endianess = Endianness::Little)
	{
		b_T<T> tmp;
