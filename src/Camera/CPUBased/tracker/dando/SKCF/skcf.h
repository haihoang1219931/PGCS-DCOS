...
struct TObj {
    ...
    cv::Size2i    windowSize;   // Optimal window size for fft performance
    cv::Size2d          size;   // Current size of the object
    cv::Point2f       center;   // Center location of the object
    vector<cv::Mat> model_xf;   // Fourier Domain: model of the tracking obj.
    cv::Mat     model_alphaf;   // Fourier Domain: Kernel Ridge Regression.
};
...
struct KFlowConfigParams {
    ...
    cv::Size winLK = cv::Size(win_size_lk, win_size_lk);
    cv::TermCriteria criteria = cv::TermCriteria(cv::TermCriteria::COUNT + cv::TermCriteria::EPS, 20, 0.03);
...
class KFlow
{
    ...
    public:
        vector<cv::Point2f> _pts;
        vector<float> _weights;
        cv::Mat _curr;
        double _scale;
        KFlow(): _params(), _pts(), _curr(), _scale(1.)
        {}
        static void toGray(const cv::Mat &input, cv::Mat &output)
        {
            if (input.channels() == 3) {
        ...
        static void toBGR(const cv::Mat &input, cv::Mat &output)
        {
            if (input.channels() == 1) {
        ...
        void extractPoints(const cv::Mat &frame, const cv::Size2d size)
        {
            toGray(frame, _curr);
            // === hainh comment here
                          ...
                          const cv::Point2f &shift)
        {
            ...
            if (_pts.size() > 0) {
                ...
                for (size_t i = 0; i < to.size(); ++i) {
                    cv::Point2f _tmp(_pts[i].x - (to[i].x + shift.x * _scale),
    ...
    public:
        static void extractPoints(const cv::Mat &patch,
                                  ...
                                  vector<float> &weights)
        {
            ...
            cv::Point tl(max(0.0, patch.cols / 2.0 - floor(size.width / 2.0)),
                         max(0.0, patch.rows / 2.0 - floor(size.height / 2.0)));
            cv::Point br(tl + cv::Point(floor(size.width), floor(size.height)));
            cv::Mat mask = cv::Mat::zeros(patch.size(), CV_8UC1);
            rectangle(mask, tl, br, cv::Scalar(255), CV_FILLED);
            vector<cv::Point2f> _tmp;
            ...
            for (int i = 0; i < POINTS ; ++i) {
                ...
                weights.push_back(_weight);
                points.push_back(cv::Point2f(nX, nY));
            }
         ...
         */
        static void flowForward(const cv::Mat &I,
                                const cv::Mat &J,
                                vector<cv::Point2f> &from,
                                vector<cv::Point2f> &to,
                                const KFlowConfigParams &p);

        static void flowForwardBackward(const cv::Mat &I,
                                        const cv::Mat &J,
                                        vector<cv::Point2f> &from,
                                        vector<cv::Point2f> &to,
                                        const KFlowConfigParams &p);
         ...
         */
        static void transform(cv::Rect_<float> &B,
                              cv::Rect_<float> &BNew,
         ...
         */
        static double transform(const vector<cv::Point2f> &start,
                                const vector<cv::Point2f> &tracked,
                                cv::Point2f &shift,
                                const KFlowConfigParams &p);
    ...
    private:
         ...
         */
        static void NORM2(vector<cv::Point2f> &ptsI,
                          vector<cv::Point2f> &ptsJ,
...
class KTrackers
{
    ...
    public:
        ...
        void show_target();
        void show_info();
        void setArea(const cv::RotatedRect &rect);
        void getTrackedArea(vector<cv::Point2f> &pts);
        void processFrame(const cv::Mat &frame);
        ...
        void getTrackedPoints(vector<cv::Point2f> &pts)
        {
            pts.clear();
    ...
    private:
        template<typename T>
        static void gaussianResponse(cv::Mat &input, double _a, double _b, double _c)
        {
            T *data = (T *)input.data;

            for (size_t i = 0; i < input.rows * input.cols; ++i) {
                T &pxl = data[i];
                pxl = cv::saturate_cast<T>(exp(_a * max(0., (_b - 2 * pxl) / _c)));
            }
        }

        template<typename T>
        static void polynomialResponse(cv::Mat &input, double _N, double _a, double _b)
        {
            T *data = (T *)input.data;

            for (size_t i = 0; i < input.rows * input.cols; ++i) {
                T &pxl = data[i];
                pxl = cv::saturate_cast<T>(pow(pxl / _N + _a, _b));
            }
        }

        static void getFeatures(const cv::Mat &patch,
                                const ConfigParams &params,
                                const cv::Mat &windowFunction,
                                vector<cv::Mat> &features);

        static void getPoints(const cv::Mat &image,
                              const cv::Mat &patch,
                              const ConfigParams &params,
                              const TObj &obj,
                              vector<cv::Point2f> &points,
        ...
        static void fft2(vector<cv::Mat> &fft2, const ConfigParams &params);//inplace
        static void fft2(const vector<cv::Mat> &features, vector<cv::Mat> &fft2, const ConfigParams &params);
        static void fft2(cv::Mat &fft2, const ConfigParams &params); //inplace
        static void fft2(const cv::Mat &features, cv::Mat &fft2, const ConfigParams &params);
        ...
        static void gaussian_correlation(const vector<cv::Mat> &xf,
        ...
        //   OUTPUT:
        //        kf  in frequency domain
        static void linear_correlation(const vector<cv::Mat> &xf,
                                       const vector<cv::Mat> &yf,
                                       cv::Mat &kf);
        ...
        //   OUTPUT:
        //        kf  in frequency domain
        static void polynomial_correlation(const vector<cv::Mat> &xf,
                                           const vector<cv::Mat> &yf,
                                           const ConfigParams &params,
                                           cv::Mat &kf);
        ...
        static double fastDetection(const cv::Mat &modelAlphaF,
                                    const cv::Mat &kzf,
                                    cv::Point &location);


        static void  getPatch(const cv::Mat &image,
        ...
        static void gaussian_shaped_labels(float sigma, const cv::Size &sz, cv::Mat &labels);
        static void gaussian_shaped_labels(float sigmaW, float sigmaH, const cv::Size &sz, cv::Mat &labels);

        //  Filtering window
        static void  gaussianWindow(const cv::Size &sz,
                                    float sigmaW,
                                    float sigmaH,
                                    cv::Mat &filter);

        //  Filtering window
        static void  hannWindow(const cv::Size &sz, cv::Mat &filter);

        //  Equation for fast trainning
        static void fastTraining(const cv::Mat &yf,
                                 const cv::Mat &kf,
                                 const ConfigParams &params,
                                 cv::Mat &alphaf);
        //  Learning the modle
        static void learn(vector<cv::Mat> &modelXf, const vector<cv::Mat> &xf,
        ...
        static void divSpectrums(cv::InputArray _srcA, cv::InputArray _srcB,
                                 cv::OutputArray _dst, int flags, bool conjB = false , double lambda = 1e-4);
        //  Sum all the real values of the spectrum.
        static double sumSpectrum(const cv::Mat &mat, const ConfigParams &params);
};
