...
void calculateHistogram(cv::Mat im, cv::Mat& averageX, cv::Mat& averageY, cv::Mat& averageX2, cv::Mat& averageY2, vector<float>& LL, vector<float>& AA, vector<float>& BB, cv::Mat& histogram,
...
int precomputeParameters(cv::Mat histogram, vector<float> LL, vector<float> AA, vector<float> BB, int numberOfPixels, vector<int>& reverseMap, cv::Mat& map, cv::Mat& colorDistance,
                        ...
                        cv::Mat& SM, cv::Mat& saliency)
{
    ...
    std::cout << " before saliency " << saliency << std::endl;
    std::cout << " before size " << saliency.cols << std::endl;
    std::cout << " before size width " << saliency.rows << std::endl;
    minMaxLoc(saliency, &minVal, &maxVal);
    saliency = saliency - minVal;
    saliency = 255 * saliency / (maxVal - minVal) + 1e-3;
    std::cout << " after saliency " << saliency << std::endl;
    minMaxLoc(saliency, &minVal, &maxVal);
...
cv::Rect computeThresMap(cv::Mat& im)
{
    cv::Rect outrect;
    ...
    cv::Mat binaryim;
    cv::threshold(im, binaryim, 200, 255, cv::ADAPTIVE_THRESH_MEAN_C);
    cv::Mat kernel = cv::getStructuringElement(CV_SHAPE_ELLIPSE, cv::Size(5, 5));
    cv::dilate(binaryim, binaryim, kernel);
    cv::Mat labelim, stats, centroids;
    int label = cv::connectedComponentsWithStats(binaryim, labelim, stats, centroids, 4);

    if (label == 1)
    {
        outrect = cv::Rect(0, 0, im.cols, im.rows);
    }
    else
    {
        ...
        outrect = cv::Rect(rect.at<int>(0), rect.at<int>(1), rect.at<int>(2), rect.at<int>(3));
    }
