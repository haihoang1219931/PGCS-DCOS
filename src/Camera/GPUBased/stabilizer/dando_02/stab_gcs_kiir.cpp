...
namespace stab_gcs_kiir
{
    ...
    ////! \return
    ////!
    int vtx_KIIRStabilizer::run(const vtx_image &input_rgb, cv::Mat &_stab_data, float *gmeData)
    {
        ageDelay();
        //============= Extract key points from current frame
        vtx_mat tmp_img;
        //        cv::cvtColor(input_rgb, _curr_gray, cv::COLOR_RGB2GRAY);
        _curr_gray = input_rgb;
        ...
        //
        // Obtain GME data - DANDA GME
        if( gmeData )
        {
            gmeData[0] = (float)_T.at<double>(0, 0);
            gmeData[1] = (float)_T.at<double>(0, 1);
            gmeData[2] = (float)_T.at<double>(0, 2);
            gmeData[3] = (float)_T.at<double>(1, 0);
            gmeData[4] = (float)_T.at<double>(1, 1);
            gmeData[5] = (float)_T.at<double>(1, 2);
            gmeData[6] = (float)_T.at<double>(2, 0);
            gmeData[7] = (float)_T.at<double>(2, 1);
            gmeData[8] = (float)_T.at<double>(2, 2);
        }
...
//        printf( "\n" );printf( "\n" );
        ...
        cv::Mat affine_T(2, 3, CV_64FC1, cv::Scalar::all(0));
