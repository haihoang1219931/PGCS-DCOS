...
void VPreprocess::run()
{
    ...
    while (m_running) {
        ...
        if (m_enStab) {
            if (m_stabilizer->run(grayImg, stabMatrix) != SUCCESS) {
                printf("===> No keypoints\r\n");
            }
        }

        // add stab Matrix
        h_stabMat[0] = stabMatrix.at<double>(0, 0);
        h_stabMat[1] = stabMatrix.at<double>(0, 1);
        h_stabMat[2] = stabMatrix.at<double>(0, 2);
        h_stabMat[3] = stabMatrix.at<double>(1, 0);
        h_stabMat[4] = stabMatrix.at<double>(1, 1);
        h_stabMat[5] = stabMatrix.at<double>(1, 2);
        h_stabMat[6] = 0.f;
        h_stabMat[7] = 0.f;
        h_stabMat[8] = 0.f;
        ...
        processImgItem.setStabMatrix(stabMatrix);
