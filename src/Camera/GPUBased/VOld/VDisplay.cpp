...
void VDisplay::init()
{
    ...
    std::string plate_cfg_file = "../Controller/Video/Clicktrack/yolo-setup/yolov3-tiny.cfg";
    std::string plate_weights_file = "../Controller/Video/Clicktrack/yolo-setup/yolov3-tiny_best.weights";
    m_detector = new Detector(cfg_file, weights_file);
    m_vODWorker->setDetector(m_detector);
    m_clicktrackDetector = new Detector(plate_cfg_file, plate_weights_file);
