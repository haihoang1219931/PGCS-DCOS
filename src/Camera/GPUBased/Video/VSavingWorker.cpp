...
bool VSavingWorker::initPipeline()
{
    ...
    //    m_pipeline_str = "appsrc name=mysrcSave ! nvvidconv ! video/x-raw(memory:NVMM),format=I420,width="
    //            + std::to_string(m_width) + ",height=" + std::to_string(m_height) + " ! omxh264enc preset-level=3 control-rate=2 bitrate=" + std::to_string(m_bitrate)
