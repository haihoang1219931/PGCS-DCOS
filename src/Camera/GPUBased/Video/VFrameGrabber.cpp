                                  ...
                                  gpointer _uData)
{
    ...
    case GST_MESSAGE_ERROR: {
        ...
        Q_EMIT pipelineError(error->code);
        g_error_free(error);
        g_main_loop_quit(loop);
        break;
    }
...
bool VFrameGrabber::initPipeline()
{
    ...
    m_pipelineStr =
        ...
        "video/x-h265,stream-format=byte-stream ! nvh265enc ! glcolorconvert ! video/x-raw(memory:GLMemory),format=BGRA ! " +
    ...
    //                    "h265parse ! nvh265enc ! glcolorconvert ! video/x-raw(memory:GLMemory),format=BGRA ! appsink name=mysink sync=true";
...
index_type VFrameGrabber::readBarcode(const cv::Mat &_rgbImg)
{
    index_type res = 0;
