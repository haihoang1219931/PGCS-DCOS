...
/*=================================================
 *
 *  BGRA to I420 conversion device function
 *
 *===============================================*/
__global__ void cuda_bgraToI420( const unsigned char *d_bgra, unsigned char *d_i420, int width, int height )
{
    int idx  = blockIdx.x * blockDim.x + threadIdx.x;
    int len = width * height;
    ...
    if( idx < len )
    {
        ...
        if( (row & 0x01) && (col & 0x01) )
        {
            ...
            int avgR = (r + (int)d_bgra[rgbIdx1] + (int)d_bgra[rgbIdx2] + (int)d_bgra[rgbIdx3]) >> 2,
                avgG = (g + (int)d_bgra[rgbIdx1+1] + (int)d_bgra[rgbIdx2+1] + (int)d_bgra[rgbIdx3+1]) >> 2,
                avgB = (b + (int)d_bgra[rgbIdx1+2] + (int)d_bgra[rgbIdx2+2] + (int)d_bgra[rgbIdx3+2]) >> 2;
...
/**
 * @brief gpu_bgraToI420
 */
cudaError_t gpu_bgraToI420( const unsigned char *d_bgra, unsigned char *d_i420, int width, int height )
{
    if( d_i420 == NULL || d_bgra == NULL )
        return cudaErrorInvalidDevicePointer;

    int numel = width * height;
    cuda_bgraToI420<<<(numel+THREAD_PER_BLOCK-1) / THREAD_PER_BLOCK, THREAD_PER_BLOCK>>>( d_bgra, d_i420, width, height );
    cudaDeviceSynchronize();
                                            ...
                                            const cv::RotatedRect &_rotRect, const float _scale )
{
    ...
    {
        printf( "! ERROR: %s:%d: err = %d: Invalid image or patch dimension\n", __FUNCTION__, __LINE__, cudaErrorInvalidValue );
        return cudaErrorInvalidValue;
    }

    if( _scale <= 0 )
    {
        printf( "! ERROR: %s:%d: err = %d: Invalid scale value\n", __FUNCTION__, __LINE__, cudaErrorInvalidValue );
        return cudaErrorInvalidValue;
    }
                           ...
                           const int _width, const int _height, const int _depth, const int _binNum )
{
    ...
    {
        printf( "! ERROR: %s:%d: err = %d: Invalid image dimension or format\n", __FUNCTION__, __LINE__, cudaErrorInvalidValue );
        return cudaErrorInvalidValue;
    }

    if( _binNum <= 0 )
    {
        printf( "! ERROR: %s:%d: err = %d: Invalid number of histogram bins\n", __FUNCTION__, __LINE__, cudaErrorInvalidValue );
        return cudaErrorInvalidValue;
    }
    ...
    if( err != cudaSuccess )
    {
        printf( "! ERROR: %s:%d: err = %d: Failed to set value for device memory\n", __FUNCTION__, __LINE__, err );
        return err;
    }
    ...
    if( err != cudaSuccess )
    {
        printf( "! ERROR: %s:%d: err = %d: Failed to allocate device memory\n", __FUNCTION__, __LINE__, err );
        return err;
    }
    ...
    {
        cudaFree( sum );
        printf( "! ERROR: %s:%d: err = %d: Failed to set value for a memory region\n", __FUNCTION__, __LINE__, err );
        return err;
    }
...
cudaError_t gpu_histSimilarity( const float *_hist1, const float *_hist2, float *_score, const int _len )
{
    ...
    {
        printf( "! ERROR: %s:%d: err = %d: NULL pointer(s)\n", __FUNCTION__, __LINE__, cudaErrorIllegalAddress );
        return cudaErrorIllegalAddress;
    }
    ...
    usleep(50);
    if( err != cudaSuccess )
        printf( "! ERROR: %s:%d: err = %d: Failed to copy data from device to host\n", __FUNCTION__, __LINE__, err );
    cudaFree( bhatSim );
