...
TreronGimbalPacketParser::TreronGimbalPacketParser(QObject* parent) :
    QObject(parent)
{
    //receivedBuffer = vector<unsigned char>(1024);
}
TreronGimbalPacketParser::~TreronGimbalPacketParser(){

}

void TreronGimbalPacketParser::Push(vector<unsigned char> data){
    receivedBuffer.insert(receivedBuffer.end(),data.begin(),data.end());
}

void TreronGimbalPacketParser::Push(unsigned char* data, int length){
...
void TreronGimbalPacketParser::Parse(){
...
//            }
//            printf("\n");
            Q_EMIT TreronGimbalPacketParser::gimbalPacketParsed(key, data);
...
void TreronGimbalPacketParser::Reset(){
