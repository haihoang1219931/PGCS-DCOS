...
void SBusGimbal::setJoystick(JoystickThreaded* joystick){
    if(joystick == nullptr) return;
    m_joystick = joystick;
    connect(m_joystick,&JoystickThreaded::axisValueChanged,this,&SBusGimbal::handleAxisValueChanged);
    connect(m_joystick,&JoystickThreaded::buttonStateChanged,this,&SBusGimbal::handleButtonStateChanged);
    //    m_context->m_hfov[m_context->m_sensorID] = 62.9f;
}
void SBusGimbal::handleButtonStateChanged(int buttonID, bool pressed){
...
void SBusGimbal::handleAxisValueChanged(int axisID, float value){
    if(!m_joystick->pic()){
...
void SBusGimbal::connectToGimbal(Config* config){
    if(config == nullptr)
        return;
    ...
    m_sensor->connectToHost(m_config->value("Settings:SensorIP:Value:data").toString(),
                            m_config->value("Settings:SensorPortIn:Value:data").toInt());
    connect(m_timerQueryZoom,&QTimer::timeout,this,&SBusGimbal::sendQueryZoom);
    connect(m_sensor,&SensorController::dataReceived,this,&SBusGimbal::handleQuery);
    m_timerQueryZoom->start();
    setEOZoom("",1);
}
void SBusGimbal::disconnectGimbal(){
    if(m_gimbal==nullptr)
        return;
}
...
void SBusGimbal::setEOZoom(QString command, float value){
    if(command == "ZOOM_IN"){
...
void SBusGimbal::setIRZoom(QString command){
    float zoomDigital = 1;
    if(command == "1x"){
        zoomDigital = 1;
    }else if(command == "2x"){
...
void SBusGimbal::setGimbalRate(float panRate,float tiltRate){
...
void SBusGimbal::snapShot(){
    if(m_videoEngine!=nullptr){
        m_videoEngine->capture();
        Q_EMIT functionHandled("Snapshot has been taken!");
    }
}
void SBusGimbal::changeTrackSize(float trackSize){
    if(m_videoEngine!=nullptr){
        m_videoEngine->changeTrackSize((int)trackSize);
    }
}
void SBusGimbal::setDigitalStab(bool enable){
    if(m_videoEngine!=nullptr){
...
void SBusGimbal::handleQuery(QString data){
...
void SBusGimbal::enableDigitalZoom(bool enable){
